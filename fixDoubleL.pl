...
# while ( readdir $dh ) {
#     chomp;
#     next if $_ eq '.' or $_ eq '..' or $_ !~ m/\.(html|txt)$/;    
#     my $textfile = $_;
...
#     while (<FH>) {
#             my $line = $_;
#             $line = $line =~ s/^\s+|\s+$//gr; # trim spaces
#             $line = $line =~ s/\s*\([^\)]+?\)\s*//r; # remove nicknames in brackets
#             push(@comments, $line) if $line =~ m/.{120,}/;
#         }
#         $i++;
#         next;
#     }
